...
3. **Deploy** and check the probes.
   ...
   **Windows: skip `app_deploy_draft` entirely — it cannot pass there.**
   Its `sql_smoke` preflight compares OS-separator paths against
   forward-slash config keys and they can never match (see "Known issue"
   below for the exact bug). Calling it anyway costs one build/retry cycle
   per attempt for a check that is structurally guaranteed to fail. Do this
   instead:
   ...
   A green `data_layer` with no `sql_smoke` step at all means the dashboard
   is verified and live — `sql_smoke` failing on Windows is a platform
   artifact, not a signal to keep retrying the deploy.
